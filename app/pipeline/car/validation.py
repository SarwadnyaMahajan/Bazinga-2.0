...
async def validate_claim(db: AsyncIOMotorDatabase, policy_number: str, evidence_path: str, required_fields: dict) -> tuple[bool, str]:
    ...
    result is either the rejection reason (if passed=False) or the image_hash (if passed=True).
    """
    ...
    if existing:
        return False, f"This image was already submitted in claim #{existing['_id']}."
